    ...
    el.innerHTML = `
      <div class="timeline-card">
        ...
        ${imgPath ? `<img
          ...
          alt="${altText}"
          loading="lazy"
          style="width: 100%; height: auto; border-radius: 8px; margin-top: 10px; border: 1px solid var(--border);"
...
// FIXED: Data loading that handles the nested structure
  ...
  for (const path of paths) {
    try {
      ...
      if (res.ok) {
        ...
        // FIXED: Check if data has 'items' array (nested structure)
        if (jsonData.items && Array.isArray(jsonData.items)) {
          console.log(`✅ Found ${jsonData.items.length} items in nested structure`);
        ...
        // If it's something else, try to extract items
        console.warn("Unexpected data structure:", Object.keys(jsonData));
        if (jsonData.data && Array.isArray(jsonData.data)) {
          return jsonData.data;
        }
        
        throw new Error("Data structure doesn't contain an 'items' array");
      }
    } catch (e) {
...
(async function init() {
  try {
    ...
    console.log(`📅 Years: ${uniqYears(data).join(", ")}`);
